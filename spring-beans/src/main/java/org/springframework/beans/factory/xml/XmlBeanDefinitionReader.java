...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		// 1. 将resource包装成带编码格式的EncodeResource
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		// 将当前EncodeResource加入到threadlocal中, 加入失败, 说明当前encodedResource已经加载过了, 不能重复加载
		...
		try {
			// 拿到EncodeResource包装的输入流对象
			InputStream inputStream = encodedResource.getResource().getInputStream();
			try {
				// 接下来需要使用sax解析器，解析xml，所以需要将输入流包装成inputSource，inputSource是sax中表示资源的对象
			...
			// resourcesCurrentlyBeingLoaded表示当前线程正在加载的resource, 执行到这里说明resource处理完毕, 需要从set中移除
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			// 将resource转化成程序层面可以识别的有层次结构的doucument
			Document doc = doLoadDocument(inputSource, resource);
			// 将document解析成bd, 并且注册到bf中, 最终返回新注册到bf的数量
	 ...
	 * 这个方法就是将inputSource转化成程序可以识别的document对象
	 */
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		// getRegistry() 会返回咱们程序创建的BeanFactory实例
